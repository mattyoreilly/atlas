...
hard guarantees, `max_steps` and `max_runtime` are mechanically enforced:
past those budgets the agent's code execution is blocked outright.
...
`atlas()` is a one-call wrapper around an `atlas_session`, which lives in the
